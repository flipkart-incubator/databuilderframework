...
public class ContextSwitchOverHeadTest {
	...
	private final BuilderExecutor builderExecutor = new BuilderExecutor(50, 10);
	...
	public void testConcurrentExecution() throws Exception{
		...
		final DataFlowInstance instance  = new DataFlowInstance("test", dataflow);
		
		ExecutorService exec = Executors.newFixedThreadPool(100); // concurrent users
...
//				public void run() {
					try {
						DataExecutionResponse resp = se.run(instance, new DataA());
						boolean target = resp.getResponses().containsKey("J");
...
//		exec.awaitTermination(1000, unit)
